...
Dirt, grass and dirt path slabs can be placed in the top or bottom half of a
block. Placing a matching slab into the empty half combines the pair into the
related vanilla block:

- two dirt slabs become a dirt block;
- two grass slabs become a grass block;
- two dirt path slabs become a dirt path block.
...
Slabs can be waterlogged. Waterlogged grass and dirt path slabs turn into
waterlogged dirt slabs. A waterlogged slab cannot be flattened or changed by
placing turf on it.

Use a compatible shovel on a dirt or grass slab to make a lowered dirt path
slab. The slab stays in the same half of the block and the shovel takes the
normal durability damage. Dirt path slabs do not spread grass or support
plants.

Grass and dirt slabs show a snow covered top and snowy side edges when vanilla
snow is nearby. This is a visual effect and does not add collision height.
...
The turf recipe works in the player's 2 by 2 crafting grid and a crafting
table. It appears in the recipe book after the player obtains a grass block or
grass slab. Dirt path slabs have no survival crafting recipe.

Grass slabs normally drop dirt slabs and drop themselves with Silk Touch. Dirt
path slabs always drop dirt slabs, including when Silk Touch is used.
...
feature. See [Configuration](CONFIGURATION.md) to disable it.
