...
exact Forge `1.21.1-52.1.0` development dependency and is excluded from every
Sky's Grass Slabs publication artifact.
...
Forge `1.21.1-52.1.0` compiles without source compatibility edits. Its explicit
...
`ORESPAWN_MAVENIZER_OFFLINE=true`. Mavenizer itself runs on Java 25; Sky's
Grass Slabs and Minecraft 1.21.1 continue to compile for Java 21.
...
6. Run this project's `verifyMavenizerCompatibilityFixture` task. The build
   script contains the authoritative checksums and also verifies the embedded
